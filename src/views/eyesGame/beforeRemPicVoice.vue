...
export default {
    setup() {
        ...
        onUnmounted(() => {
            (getHttpConfig.value as elementsType[]).forEach((item, index) => {
                console.log(item);
                (function (index) {
                    console.log(item)
                    if (fontTimerArr.value[index] != undefined) {
                        window.clearTimeout(fontTimerArr.value[index] as NodeJS.Timeout);
                    }
                })(index);
            });
        })
