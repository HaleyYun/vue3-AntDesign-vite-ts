<template>
     <div class="task-l">
        <div class="canvas">
            <CanvasFunc :ref="'goss' + canvasObj.pageIndex" :canvasObj="canvasObj" :count="count"></CanvasFunc>
            <div class="boxBg"><img :src="optionData?.imgPathList[0]" :style="{transform : `rotateX(${count}deg)`}"></div>
        </div>
...
export default {
    ...
    setup(props,{emit}) {
        ...
        onMounted(() => {
            nextTick(() => {
                proxy.$refs[
                    "goss" + state.canvasObj.pageIndex
                ].pencilBtn("#5766fe");
            });
        })
        ...
        const changePenColor = (color) => {
            ...
            console.log(JSON.parse(setMocaData.mocaAllDataList) +'画布1')
        ...
        const uploadBaesUrl = (val,bol) => {
            ...
            proxy.$refs["goss" + state.canvasObj.pageIndex].returnBaseUrl()
            .then((res) => {
