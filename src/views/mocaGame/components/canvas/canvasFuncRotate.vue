<template>
    <div class="canvas-page">
        <div ref="canvasDad" class="canvas-box">
            <!-- <button @click="returnBaseUrl">返回base64</button> -->
            <img v-if="showPicUrl" :class="csListObj.picCanvas" :id="'picUser ' + picListClass" ref="imgDom" :src="showPicUrl" alt="" class="img-css" />
            <canvas style="display: block" id="drawCanvasClass" :width="csListObj.csWidth" :height="csListObj.csHeight"></canvas>
        </div>
        <!--以下为合成图片的画布，尺寸大小同上-->
        <canvas v-if="isOpenCanvas" :id="concatPicCanvasClass" :width="csListObj.csWidth" :height="csListObj.csHeight"></canvas>
    </div>
</template>
<script>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        // console.log(store.state.canvasPageToTal)
        ...
        // watch(() => store.state.swipeIndex, async (val, oldVal) => {
        //     store.state.drawingState.forEach(item => {
        //         if (setAttr.csListObj.pageIndex === item.pageIndex + 1) {
        //             setAttr.drawCanvasClass = store.state.drawingState[item.canvasIndex].aaC
        //             setAttr.concatPicCanvasClass = store.state.drawingState[item.canvasIndex].bbC
        //             setAttr.showPicUrl = store.state.drawingState[item.canvasIndex].isNeedPic ? setAttr.csListObj.imgUrla : ''
        //             setAttr.isAppendPic = setAttr.csListObj.isNeedImg
        //             setAttr.getPageIndex = val
        //             setAttr.asideIsPicShow = store.state.drawingState[item.canvasIndex].leftPic
        //             setAttr.picListClass = store.state.drawingState[item.canvasIndex].ccC
        //         }
        //     })
        // }, {
        //     deep: true,
        ...
        // })
        onMounted(() => {
            // console.log(store.state.drawingState)  根据index索引值，对应赋值类名
        ...
        const resizeCanvas = () => {
            // 设置canvas宽高
            // console.log(canvasDad.value.clientWidth)
            setAttr.csListObj.csWidth = canvasDad.value.clientWidth;
            setAttr.csListObj.csHeight = canvasDad.value.clientHeight;
        };
        const computedDistance = () => {
            // 计算距离
            let infectDistance = document.getElementById('drawCanvasClass').getBoundingClientRect().left
            return infectDistance;
        };
        const computedDistance2 = () => {
            // 计算距离
            let infectDistance = document.getElementById('drawCanvasClass').getBoundingClientRect().top
            return infectDistance;
        };
        ...
        const initCanvas = async () => {
            // 初始化canvas
            await resizeCanvas();
            setAttr.setCanvas = document.getElementById('drawCanvasClass')
            ...
            let getCanvasWidth =
                document.getElementById('drawCanvasClass').clientWidth;
            let getCanvasHeight =
                document.getElementById('drawCanvasClass').clientHeight;
            // console.log(setAttr.getLeft, setAttr.getTop)
        ...
        const returnBaseUrl = async () => { // 保存画板
            setAttr.isOpenCanvas = true;
            const canvasDom = document.getElementById('drawCanvasClass'); //获取canvas DOM对象
        ...
        const pencilBtn = (penColor) => {
            ...
            // let strokeStyle = setAttr.csListObj.pencilColor
        ...
        const appendPic = (url) => {
            ...
            img.src = url;
            img.onload = () => {
                setAttr.ctx.drawImage(
                    img,
                    setAttr.csListObj.imgX,
                    setAttr.csListObj.imgY,
                    setAttr.csListObj.imgWidth,
                    setAttr.csListObj.imgHeight
                );
            };
